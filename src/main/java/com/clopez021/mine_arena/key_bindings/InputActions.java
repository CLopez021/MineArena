...
//import com.clopez021.mine_arena.packets.PlaceModelPacket;
//import com.clopez021.mine_arena.packets.UndoModelPacket;
...
import static com.clopez021.mine_arena.items.custom.HammerModes.*;
import static com.clopez021.mine_arena.key_bindings.KeyBindings.*;

public class InputActions {
    public static void mouseScrollEvent(InputEvent.MouseScrollingEvent event) {
        Player player = Minecraft.getInstance().player;
        if (player == null || !player.getMainHandItem().getItem().equals(ModItems.MODEL_HAMMER.get()) || MineArena.model == null) return;
    ...
    public static void checkKeys() {
        ...
        boolean modelLoaded = MineArena.model != null;
    ...
    public static void toggleViewMode(Player player) {
        if (MineArena.model == null) return;
        switch (viewMode) {
            case ViewMode.BOX -> {
            ...
            case ViewMode.WIREFRAME -> {
                ...
                MineArena.model.applyScale(1);  // Cause update to calculate block positions
    ...
    public static void handleMouseScroll(float value) {
        ...
        switch (transformMode) {
            case TRANSLATE -> MineArena.model.move(selectedAxis.name, value);
            case ROTATE -> MineArena.model.applyAxisRotation(selectedAxis.name, 5 * value);
            case SCALE -> {
                Vector3f size = new Vector3f(MineArena.model.maxCorner).sub(MineArena.model.minCorner);
                Vector3f scale = new Vector3f();
                if (selectedAxis == Axis.ALL)
                    scale.set(2/size.get(size.maxComponent())*value);
                else
                    scale.setComponent(selectedAxis.component, 2/(MineArena.model.maxCorner.get(selectedAxis.component) - MineArena.model.minCorner.get(selectedAxis.component)) * value);
                MineArena.model.setScale(MineArena.model.scale.x + scale.x, MineArena.model.scale.y + scale.y, MineArena.model.scale.z + scale.z);
            }
        }
    }
}