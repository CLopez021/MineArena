    ...
    try {
      ...
      if (RecorderManager.isRecording(playerUuid)) {
        command
            ...
            .sendFailure(
                Component.literal(
                    "Error: You are already being recorded! Please wait for the current 5-second recording to finish."));
        return 0;
      }
      ...
      if (success) {
        command
            ...
            .sendSuccess(
                () ->
                    Component.literal(
                        "Started 5-second audio recording for command: " + commandArg),
                false);
        command
            .getSource()
            .sendSuccess(
                () -> Component.literal("Recording will automatically save in 5 seconds..."),
                false);
        return 1;
      } else {
        command
            ...
            .sendFailure(
                Component.literal(
                    "Error: Failed to start recording. Make sure Simple Voice Chat is installed and working."));
        return 0;
      }
