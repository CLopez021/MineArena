...
public class WandScreens {
    ...
    public static void startLoadingWithTimeout(long timeoutMs) {
        ...
        if (mc.player != null) {
            mc.player.displayClientMessage(Component.literal("Brewing spell — may take a while").withStyle(ChatFormatting.LIGHT_PURPLE), true);
        }
    }
    ...
    public static void onSpellCompleteClient() {
        ...
        Minecraft mc = Minecraft.getInstance();
        if (mc.player != null) {
            mc.player.displayClientMessage(Component.literal("Spell completed").withStyle(ChatFormatting.LIGHT_PURPLE), true);
        }
    }

    @Mod.EventBusSubscriber(modid = MineArena.MOD_ID, value = Dist.CLIENT, bus = Mod.EventBusSubscriber.Bus.FORGE)
