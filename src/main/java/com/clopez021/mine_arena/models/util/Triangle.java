...
/** An object representing a triangle in space. Contains three Points. */
public class Triangle {
  ...
  public Triangle transformed(Matrix4f matrix) {
    return new Triangle(
        this.v1.transformed(matrix), this.v2.transformed(matrix), this.v3.transformed(matrix));
  }
  ...
  public Set<Point> getBlockPoints(Set<Point> points) {
    for (Point v4 : v1.line(v2)) for (Point p : v4.line(v3)) points.add(p.blockPoint());
    return points;
  }
}
