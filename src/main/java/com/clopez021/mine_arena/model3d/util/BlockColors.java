...
/**
 * A class to read through a jar file and extract the names and colors of valid blocks. Valid blocks
 * are blocks that can be used to approximate a pixel color.
 */
public class BlockColors {
  ...
  public static Map<String, Integer> getBlockColors(JarFile jarFile) throws IOException {
    ...
    for (JarEntry entry : blockStates) {
      ...
      if (firstModel != null
          && isCubeModel(jarFile, firstModel)
          && isValidBlock(entry)
          && isValidName(entry)) {
        String blockName =
            entry.getName().replace("assets/minecraft/blockstates/", "").replace(".json", "");
  ...
  /**
   * Searches through the JarFile and returns a list of entries that represent blockstate json
   * files.
   *
   * @param jarFile A Minecraft jar file.
  ...
  private static List<JarEntry> getBlockStateEntriesFromJar(JarFile jarFile) {
    ...
    while (entries.hasMoreElements()) {
      ...
      if (entry.getName().startsWith("assets/minecraft/blockstates/")) blockStates.add(entry);
    }
    return blockStates;
  }

  /**
   * Searches through the blockstate file and returns an entry to the first model specified in the
   * file.
   *
   * @param jarFile A Minecraft jar file.
   ...
   * @throws IOException The JarFile could not be read.
   */
  private static JarEntry getFirstModelEntry(JarFile jarFile, JarEntry blockState)
      throws IOException {
  ...
  private static boolean isValidBlock(JarEntry entry) {
    ResourceLocation id = ResourceLocation.tryParse(entry.getName());
    Block block = ForgeRegistries.BLOCKS.getValue(id);
    if (block == null || block.defaultBlockState().isAir()) return false;
    List<Class<?>> classes =
        List.of(
            FallingBlock.class,
  ...
  private static boolean isValidName(JarEntry entry) {
    String[] invalidStrings = new String[] {"_ore", "_coral_block"};
